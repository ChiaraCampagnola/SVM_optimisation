...
class SMO:
    def __init__(self, tol, C, kernel_fun, train_X, train_y, max_passes, speed='fast'):
        ...
        # For fast SMO
        # Flag to indicate whether we are selecting alpha_1 among ALL alphas that break KKT
        # or only the restricted ones
        # Restricted alphas = alphas that break KKT AND are not at the boundary (0 or C)
        self.restricted_loop = False
    ...
    def candidate_alphas(self, alpha, kernel, b):
        ...
        for i in range(num_training_examples): # Iterate through alphas
            ...
            # Check it breaks KKT
            if (self.train_y[i]*self.E[i] < -self.tol and alpha[i] < self.C) or (self.train_y[i]*self.E[i] > -self.tol and alpha[i] > 0):
                candidates.add(i)
    ...
    def find_alpha_2(self, i, kernel, b, non_bound_alphas):
        pred_y_i = predict(alpha, kernel[:,i], b, self.train_y)
        E_i = pred_y_i - self.train_y[i] # Error
    ...
    def smo_fast(self):
        ...
        while passes < self.max_passes:
            ...
            # Create two lists of candidates: all KKT breaking, and non-boundary
            candidate_alphas, candidate_non_boundary_alphas = self.candidate_alphas(alpha, kernel, b)
            
            if not self.restricted_loop:
                # Iterate through all KKT breaking alphas -> alphas
                for i in candidate_alphas:
                    j = find_alpha_2(i, kernel, b, non_bound_alphas)
                    
                    
                self.restricted_loop = True # Go to restricted loop
            else:
                # Keep going through all KKT breaking alphas
                # not at boundary until there are no more -> non_boundary_alphas
                while candidate_non_boundary_alphas:
                    pass
                
                # Find set of alphas
                
                # While the set is not empty keep going through it
                
                self.restricted_loop = False # Go back to larger loop
